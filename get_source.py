...
class GetSource(object):
    ...
    url = 'https://codeload.github.com/HennyNB/MultiFunctionalInputDevice/zip/refs/heads/main?token=BKVNIHCXLCEDFQAMQ77PP7THKVE5C'
    response = requests.get(url)

    structure = url.split('/')
    branch = structure[8].split('?token=')

    path = config.source_path + '\\' + structure[4] + '-' + branch[0] + '\\'
