...
class Config(object):
    ...
    doc_path = internal_path + '__docs__' + '\\'
    log_path = internal_path + '__logs__' + '\\'
    data_path = internal_path + '__inputs__' + '\\'
    source_path = internal_path + '__sources__' + '\\'

    zip_file_path = internal_path + 'file.zip'
    ...
    def choices_title(self):
        ...
        self.text_middle(text='[5]： 获取项目源码')
        self.split()
    ...
    def be_getting(self):
        self.text_middle(text='正在获取')

    def have_gotten(self):
        self.text_middle(text='下载成功')

    def download_error(self):
        self.text_middle(text='【错误】下载失败，请检查网络连接')
    ...
    def waiting_to_writing(self, flag: str):
        self.text_middle()

        for item in range(3):
            os.system('cls')

            self.function_title(flag=flag)
            self.text_middle()
            self.text_middle(text='距离输入还有 0' + str(3 - item) + ' 秒')
            time.sleep(1)
